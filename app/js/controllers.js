...
}]).controller('hashedLocation', ['$scope', '$rootScope', 'retrieveLocation', 'LocationDataFetch', '$location', '$routeParams', '$q', 'runSymbolChange', 'PlaylistCreate', 'MapCreate', 'States','$sce',
function($scope, $rootScope, retrieveLocation, LocationDataFetch, $location, $routeParams, $q, runSymbolChange, PlaylistCreate, MapCreate,States, $sce) {
	...
	$scope.runCycle = function() {
		...
		if (LocationDataFetch.count==0) {
			...
			console.log($rootScope.longitudeObj_root.location.toLowerCase()+':'+location_comp.toLowerCase());
	...
	$scope.removeSong = function(id) {
		...
		for (var i = 0; i < songs.length; i++) {
			if (id == songs[i].id) {
				songs[i].closeButton = true;
				$scope.leaveOut.push(songs[i].num_id);

			} else {
				$scope.spot_arr.push(songs[i].tracks[0].foreign_id.split(':')[2])

			}
		}
		songs.spot_str = 'https://embed.spotify.com/?uri=spotify:trackset:PREFEREDTITLE:' + $scope.spot_arr.toString();
		songs.spot_str = $sce.trustAsResourceUrl($rootScope.songs.spot_str);
	...
	$scope.switchFavorite=function(id)
	{
		...
		console.log($scope.favoritesArr);
	};
...
function($scope, $routeParams, retrieveLocation,LocationDataFetch, PlaylistCreate,MapCreate, $location, $rootScope, $q, loadGenreCheckData, runSymbolChange) {
	...
	$scope.checkEra = function(start_year, end_year)
	{
		...
		{
		$rootScope.era = '&artist_start_year_after='+start_year+'&artist_end_year_after=' +$scope.d;	
		}
		else if(end_year=='')
		{
...
FavoritesControllers.controller('LoadFav', ['$scope', '$http', 'runSymbolChange', '$routeParams', '$location',
function($scope, $http, runSymbolChange, $routeParams, $location) {
	$scope.location = $routeParams.location;
