...
MusicWhereYouAreApp.directive('yearSlider', function($compile){
	return{
		...
		template:'<input type="range" ng-model="start_year" min="1890" max="{{d}}" step="1"  style="width: 200px" name="start" value="{{start_year}}"> {{start_year}} <input type="range" ng-model="end_year" min="1890" max="{{d}}"  step="1" style="width: 200px" name="end" value="{{end_year}}"> {{end_year}}'
	};
